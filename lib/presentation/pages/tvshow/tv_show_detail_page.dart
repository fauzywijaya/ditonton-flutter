  ...
  Widget build(BuildContext context) {
    return ScrollableSheet(
      ...
      scrollableContents: [
        ...
        ElevatedButton(
          onPressed: () async {
            if (!provider.isAddedToWatchlist) {
              ...
              await provider.removeWatchlist(tvShow);
            }

            final message = provider.watchlistMessage;

            if (message == ADD_MESSAGE || message == REMOVE_MESSAGE) {
        ...
        Text(
          getFormatedGenres(tvShow.genres),
        ),
        Text(
          tvShow.episodeRunTime.isNotEmpty
              ? getFormatedDurationFromList(tvShow.episodeRunTime)
              : 'N/A',
        ),
        Row(
          children: [
